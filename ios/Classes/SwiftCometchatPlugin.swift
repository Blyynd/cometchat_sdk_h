...
public class SwiftCometchatPlugin: NSObject, FlutterPlugin {
    ...
    var reactionsRequestDict: [String:ReactionsRequest] = [:]
...
//                self?.fetchPreviousMessages(args: args, result:result)
            ...
            case "fetchPreferences": self?.fetchPreferences( args: args, result: result)
            case "updatePreferences": self?.updatePreferences( args: args, result: result)
            case "resetPreferences": self?.resetPreferences( args: args, result: result)
            case "getMutedConversations": self?.getMutedConversations( args: args, result: result)
            case "muteConversations": self?.muteConversations( args: call.arguments as? [Int: Any] ?? [Int: Any](), result: result)
            case "unmuteConversations": self?.unmuteConversations( args: call.arguments as? [Int: Any] ?? [Int: Any](), result: result)
            case "registerPushToken": self?.registerPushToken( args: args, result: result)
            case "unregisterPushToken": self?.unregisterPushToken( args: args, result: result)
            case "fetchNextAndPreviousForReactionsRequest": self?.fetchNextAndPreviousForReactionsRequest(args: args, result: result)
            case "getConversationUpdateSettings": self?.getConversationUpdateSettings( args: args, result: result)
            case "updateTimezone": self?.updateTimezone(args: args, result: result)
            case "getTimezone": self?.getTimezone(args: args, result: result)
...
//        }
    ...
    private func fetchNextConversations(args: [String: Any], result: @escaping FlutterResult){
        ...
        let tags = args["tags"] as? [String] ?? []
        let includeBlockedUsers = args["includeBlockedUsers"] as? Bool ?? false
        let withBlockedInfo = args["withBlockedInfo"] as? Bool ?? false
        ...
        if( key == nil || conversationRequestDict[key!] == nil ){
            ...
            if(includeBlockedUsers){
                builder = builder.include(blockedUsers: includeBlockedUsers)            }
            if(withBlockedInfo){
                builder = builder.with(blockedInfo: withBlockedInfo)
            }
            if(!tags.isEmpty){
    ...
    private func fetchPreviousMessages(args: [String: Any], result: @escaping FlutterResult){
        ...
        if( key == nil || messageRequestDict[key!] == nil ){
            ...
            builder = builder.mentionsWithTagInfo(mentionsWithTagInfo).mentionsWithBlockedInfo(mentionsWithBlockedInfo)
    ...
    private func sendCustomMessage(args: [String: Any], result: @escaping FlutterResult){
        ...
        let updateConversation = args["updateConversation"] as? Bool ?? false
        let sendNotification = args["sendNotification"] as? Bool ?? false
        ...
        if (!text.isEmpty){
            customMessage.conversationText = text
        }
        if (updateConversation){
            customMessage.updateConversation = updateConversation
        }
        if (sendNotification){
            customMessage.sendNotification = sendNotification
        }
        ...
        if let message = message {
            ...
            if let text = message as? TextMessage {
                let map = [
                    "text" : text.text,
                    "tags" : text.tags,
                    "reactions" : text.reactions.map({ reactionCount in
                ...
                let map = [
                    ...
                    "tags" : media.tags,
                    "reactions" : media.reactions.map({ reactionCount in
                        self.getReactionCountMap(reactionCount)
                    })
                ]
                map.forEach { (key, value) in messageMap[key] = value }
                ...
                let map = [
                    ...
                    "text" : customMessage.conversationText,
                    "updateConversation" : customMessage.updateConversation,
                    "sendNotification" : customMessage.sendNotification,
                    "reactions" : customMessage.reactions.map({ reactionCount in
                        self.getReactionCountMap(reactionCount)
                    })
                ]
                map.forEach { (key, value) in messageMap[key] = value }
    ...
    private func getGroupMap(group: Group?, methodName : String = "")-> [String: Any]? {
        ...
        if let group = group {
            return [
                ...
                "tags" : group.tags,
                "isBannedFromGroup" : group.isBannedFromGroup,
                "methodName":methodName
            ]
        } else {
            return nil
    ...
    private func getMessageReactionMap(messageReaction: Reaction) -> [String:Any]{
    ...
    private func fetchNextAndPreviousForReactionsRequest(args: [String: Any], result: @escaping FlutterResult) {
            let paginationTask: Int? = args["paginationTask"] as? Int
            var key: String? = args["key"] as? String
            let limit: Int = args["limit"] as? Int ?? 10
            let messageId = args["messageId"] as? Int ?? -1
            let reaction = args["reaction"] as? String
            
            var reactionsRequest: ReactionsRequest
            
            if(key == nil || reactionsRequestDict[key!] == nil){
                var builder: ReactionsRequestBuilder = ReactionsRequestBuilder()
                if limit > 0{
                    builder = builder.set(limit:limit)
                }
                if messageId > 0 {
                ...
                if let _reaction = reaction {
                    builder = builder.set(reaction:_reaction)
                }
                reactionsRequest = builder.build()
                if(key == nil){
                    counter+=1
                    key = String(counter)
                }
                reactionsRequestDict[key!] = reactionsRequest
            }else{
                reactionsRequest = reactionsRequestDict[key!]!
            }
            if (paginationTask == 0){
                reactionsRequest.fetchNext{messageReactions in
                    let list = messageReactions.map { messageReaction in
                        self.getMessageReactionMap(messageReaction: messageReaction)
                    }
                ...
                reactionsRequest.fetchPrevious{messageReactions in
                    let list = messageReactions.map { messageReaction in
                        self.getMessageReactionMap(messageReaction: messageReaction)
                    }
                    var resultMap:[String:Any?] = [:]
    ...
    private func getConversationUpdateSettings(args: [String: Any], result: @escaping FlutterResult) {
        var conversationSettings = CometChat.getConversationUpdateSettings()
        result(conversationSettings.toDictionary())
    }
    
    public func getReactionEventMap(_ reactionEvent: ReactionEvent)-> [String:Any]{
        var reactionEventMap:[String:Any] = [:]
        if let _reaction = reactionEvent.reaction {
            reactionEventMap["reaction"] = getMessageReactionMap(messageReaction: _reaction)
        }
        reactionEventMap["receiverId"] = reactionEvent.receiverId
        if reactionEvent.receiverType == .user{
            reactionEventMap["receiverType"] = "user"
        } else if reactionEvent.receiverType == .group {
            reactionEventMap["receiverType"] = "group"
        }
        reactionEventMap["conversationId"] = reactionEvent.conversationId
        reactionEventMap["parentMessageId"] = reactionEvent.parentMessageId
        return reactionEventMap
    }
    

    private func getMutedConversations(args: [String: Any], result: @escaping FlutterResult){
        CometChatNotifications.getMutedConversations { mutedConversations in
            var r1: [Int: [String: Any]] = [:]
            for (index, _) in mutedConversations.enumerated() {
                r1[index] = mutedConversations[index].toDictionary
            }
            result(r1)
        } onError: { error in
            result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
    ...
    private func muteConversations(args: [Int: Any], result: @escaping FlutterResult){
        var mutedConversations: [CometChatNotifications.MutedConversation] = []
        for (_, value) in args {
            if let conversationMap = value as? [String: Any] {
                let rawMap = CometChatNotifications.MutedConversation.fromDictionary(conversationMap)
                guard let requestMap = rawMap.0 else {
                    result(FlutterError(code: rawMap.1?.errorCode ?? "Error"  , message: rawMap.1?.errorDescription, details: rawMap.1?.debugDescription))
                    return
                }
                mutedConversations.append(requestMap)
            }
        }
        CometChatNotifications.muteConversations(mutedConversations, onSuccess: { success in
            result(success)
          },
          onError: { error in
            result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
    ...
    private func unmuteConversations(args: [Int: Any], result: @escaping FlutterResult){
        var unmutedConversations: [CometChatNotifications.UnmutedConversation] = []
        for (_, value) in args {
            if let conversationMap = value as? [String: Any] {
                let rawMap = CometChatNotifications.UnmutedConversation.fromDictionary(conversationMap)
                guard let requestMap = rawMap.0 else {
                    result(FlutterError(code: rawMap.1?.errorCode ?? "Error"  , message: rawMap.1?.errorDescription, details: rawMap.1?.debugDescription))
                    return
                }
                unmutedConversations.append(requestMap)
            }
        }
        
        CometChatNotifications.unmuteConversations(unmutedConversations, onSuccess: { success in
            result(success)
          },
          onError: { error in
            result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
    ...
    private func registerPushToken(args: [String:Any], result: @escaping FlutterResult) {
        if let pushPlatform = args["pushPlatforms"] as? String, let pushPlatforms = pushPlatform.stringValueToPushPlatforms {
          let token:String = args["fcmToken"] as? String ?? args["deviceToken"] as? String ?? args["voipToken"] as? String ?? ""
          let providerId = args["providerId"] as? String
            
          CometChatNotifications.registerPushToken(pushToken: token, platform: pushPlatforms, providerId: providerId, onSuccess: { isSuccess in
            result(String(isSuccess))
          }, onError: {
            error in
              result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
    ...
    private func unregisterPushToken(args: [String:Any], result: @escaping FlutterResult) {
        CometChatNotifications.unregisterPushToken(onSuccess: { isSuccess in
          result(String(isSuccess))
        }, onError: {
          error in
            result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
...
extension SwiftCometchatPlugin : CometChatMessageDelegate{
    ...
    public func onInteractiveMessageReceived(interactiveMessage: InteractiveMessage) {
        guard let eventSink = self.sink else { return }
        let parsedMsg = self.getMessageMap(message: interactiveMessage,methodName: "onInteractiveMessageReceived")
        eventSink(parsedMsg)
    }
    ...
    public func onMessagesDeliveredToAll(receipt: MessageReceipt) {
            guard let eventSink = self.sink else { return }
            let parsedMsg = self.getMessageReceiptMap(messageReceipt: receipt, methodName: "onMessagesDeliveredToAll")
            eventSink(parsedMsg)
        }

        public func onMessagesReadByAll(receipt: MessageReceipt) {
            guard let eventSink = self.sink else { return }
            let parsedMsg = self.getMessageReceiptMap(messageReceipt: receipt, methodName: "onMessagesReadByAll")
            eventSink(parsedMsg)
        }
    ...
    public func onMessageReactionAdded(reactionEvent: ReactionEvent) {
        ...
        responseMap["reactionEvent"] = self.getReactionEventMap(reactionEvent)
...
extension SwiftCometchatPlugin : FlutterStreamHandler{
    ...
    private func toBaseMessage(messageMap : [String:Any?]) -> BaseMessage {
        ...
        if let mentionedUsers = messageMap["mentionedUsers"] as? [Dictionary<String,AnyObject?>]{
            message.mentionedUsers = mentionedUsers.map({ user in
                User(uid: (user["uid"] as? String) ?? "", name: (user["name"] as? String) ?? "")
            })
        }
        
        if let hasMentionedMe = messageMap["hasMentionedMe"] as? Bool{
            message.mentionedMe = hasMentionedMe
        }
        
        if let reactions = messageMap["reactions"] as? [Dictionary<String,AnyObject?>]{
            message.reactions = reactions.map({ reaction in
                var _reactionCount = ReactionCount()
                _reactionCount.reaction = reaction["reaction"] as? String ?? ""
                _reactionCount.reactedByMe = reaction["reactedByMe"] as? Bool ?? false
                _reactionCount.count = reaction["count"] as? Int ?? 0
...
extension SwiftCometchatPlugin: CometChatCallDelegate {
    ...
    private func updateTimezone(args: [String: Any], result: @escaping FlutterResult){
        let timezoneIdentifier = args["timezone"] as? String
        if let _timezoneIdentifier = timezoneIdentifier, let _timezone = TimeZone(identifier: _timezoneIdentifier) {
            CometChatNotifications.updateTimezone(_timezone, onSuccess: { success in
                result(success)
            }, onError: { error in
                result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
            })
    ...
    private func getTimezone(args: [String: Any], result: @escaping FlutterResult){

                CometChatNotifications.getTimezone { success in
...
extension SwiftCometchatPlugin {
    private func fetchPreferences(args: [String: Any], result: @escaping FlutterResult){
        CometChatNotifications.fetchPreferences { notificationPreferences in
            result(notificationPreferences.toDictionary)
        } onError: { error in
            result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
        }
    }

    private func updatePreferences(args: [String: Any], result: @escaping FlutterResult){
        let requestObj = CometChatNotifications.NotificationPreferences.fromDictionary(args)
        guard let _requestObj = requestObj.0 else {
            result(FlutterError(code: requestObj.1?.errorCode ?? "Error"  , message: requestObj.1?.errorDescription, details: requestObj.1?.debugDescription))
            return
        }
        CometChatNotifications.updatePreferences(_requestObj, onSuccess: { notificationPreferences in
            result(notificationPreferences.toDictionary)
          },
          onError: { error in
            result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
    ...
    private func resetPreferences(args: [String: Any], result: @escaping FlutterResult){
        CometChatNotifications.resetPreferences { notificationPreferences in
            result(notificationPreferences.toDictionary)
        } onError: { error in
            result(FlutterError(code: error.errorCode, message: error.errorDescription, details: error.debugDescription))
        }
