...
class CometchatPlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
    ...
    private val reactionRequestMap: MutableMap<String, ReactionsRequest> = mutableMapOf()
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        when (call.method) {
            ...
            "fetchPreferences"-> fetchPreferences(call, result)
            "updatePreferences"-> updatePreferences(call, result)
            "resetPreferences"-> resetPreferences(call, result)
            "getMutedConversations"-> getMutedConversations(call, result)
            "muteConversations"-> muteConversations(call, result)
            "unmuteConversations"-> unmuteConversations(call, result)
            "registerPushToken"-> registerPushToken(call, result)
            "unregisterPushToken"-> unregisterPushToken(call, result)
            "fetchNextAndPreviousForReactionsRequest" -> fetchNextAndPreviousForReactionsRequest(call, result)
            "getConversationUpdateSettings" -> getConversationUpdateSettings(call, result)
            "updateTimezone"-> updateTimezone(call, result)
            "getTimezone"-> getTimezone(call, result)
        ...
        when (message) {
            ...
            is CustomMessage -> map.putAll(
                hashMapOf(
                    ...
                    "tags" to message.tags,
                    "text" to message.conversationText,
                    "updateConversation" to message.willUpdateConversation(),
                    "sendNotification" to message.willSendNotification()
                )
            )
    ...
    private fun getGroupMap(group: Group ,methodName : String = ""): HashMap<String, Any?> {
        ...
        return hashMapOf(
            ...
            "tags" to group.tags,
            "isBannedFromGroup" to group.isBannedFromGroup,
            "methodName" to methodName
        )
    }
    ...
    private fun sendMediaMessage(call: MethodCall, result: Result) {
        ...
        CometChat.sendMediaMessage(
            mediaMessage,
            object : CallbackListener<MediaMessage>() {
    ...
    private fun sendCustomMessage(call: MethodCall, result: Result) {
        ...
        val updateConversation: Boolean = call.argument("updateConversation") ?: false
        val sendNotification: Boolean = call.argument("sendNotification") ?: false
        ...
        if(muid.isNotEmpty()) customMessage.muid  = muid
        ...
        if(text != null) customMessage.conversationText = text

        if(updateConversation) customMessage.shouldUpdateConversation(true)

        if(sendNotification) customMessage.shouldSendNotification(true)
        ...
        CometChat.addMessageListener(
            ...
            object : CometChat.MessageListener() {
                ...
                override fun onMessagesReadByAll(messageReceipt: MessageReceipt?) {
                    messageStreamSink?.success(getMessageReceiptMap(messageReceipt,"onMessagesReadByAll"))
                }
                override fun onMessagesDeliveredToAll(messageReceipt: MessageReceipt?) {
                    messageStreamSink?.success(getMessageReceiptMap(messageReceipt,"onMessagesDeliveredToAll"))
                }
                ...
                override fun onMessageReactionAdded(reactionEvent: ReactionEvent) {
                    ...
                    responseMap["reactionEvent"] = getReactionEventMap(reactionEvent)
        ...
        CometChat.addConnectionListener(
            ...
            object : CometChat.ConnectionListener {
                ...
                override fun onFeatureThrottled() {
                    Log.i(TAG, "WS: onFeatureThrottled")
                    val responseMap :HashMap<String,Any?> = HashMap()
                ...
                override fun onConnectionError(error: CometChatException) {
                    Log.i(TAG, "WS: onConnectionError")
                    val responseMap :HashMap<String,Any?> = HashMap()
    ...
    private fun fetchNextConversations(call: MethodCall, result: Result) {
        ...
        val withTags: Boolean = call.argument("withTags") ?: false
        val tags: List<String> = call.argument("tags") ?: emptyList()
        val includeBlockedUsers: Boolean = call.argument("includeBlockedUsers") ?: false
        val withBlockedInfo: Boolean = call.argument("withBlockedInfo") ?: false
        var key: String? = call.argument("key")
        ...
        if( key==null || !conversationRequestMap.contains(key) || ((conversationRequestMap.get(key))==null) ){
            ...
            if(includeBlockedUsers) builder = builder.includeBlockedUsers(includeBlockedUsers)

            if(withBlockedInfo) builder = builder.withBlockedInfo(withBlockedInfo)
        ...
        conversationRequest.fetchNext(object : CallbackListener<List<Conversation>>() {
    ...
    private fun getConversation(call: MethodCall, result: Result) {
        ...
        CometChat.getConversation(conversationWith, conversationType, object: CallbackListener<Conversation>(){
    ...
    private fun getUnreadMessageCountForGroup(call: MethodCall, result: Result) {
        ...
        CometChat.getUnreadMessageCountForGroup( guid, hideMessagesFromBlockedUsers , object :
            CallbackListener<HashMap<String, Int>>() {
    ...
    private fun tagConversation(call: MethodCall, result: Result) {
        ...
        CometChat.tagConversation(conversationWith, conversationType, tags,object: CallbackListener<Conversation>(){
            override fun onSuccess(conversation: Conversation) {
    ...
    private fun getMessageReactionMap(messageReaction: Reaction): HashMap<String, Serializable?> {
        return hashMapOf(
            ...
            "reactedBy" to getUserMap(messageReaction.reactedBy)
        )
    }

    private fun getReactionEventMap(reactionEvent: ReactionEvent): HashMap<String, Serializable?> {
        return hashMapOf(
                "reaction" to getMessageReactionMap(reactionEvent.reaction),
                "receiverId" to reactionEvent.receiverId,
                "receiverType" to reactionEvent.receiverType,
                "conversationId" to reactionEvent.conversationId,
                "parentMessageId" to reactionEvent.parentMessageId,
        )
    }
    ...
    private fun fetchNextAndPreviousForReactionsRequest(call: MethodCall, result: Result) {
        val paginationTask: Int = call.argument("paginationTask")!!
        val limit: Int = call.argument("limit") ?: -1
        val messageId: Int = call.argument("messageId") ?: -1
        val reaction: String? = call.argument("reaction")
        var key: String? = call.argument("key")

        lateinit var reactionRequest: ReactionsRequest

        if (key == null || !reactionRequestMap.contains(key) || ((reactionRequestMap[key]) == null)) {
            val builder: ReactionsRequest.ReactionsRequestBuilder = ReactionsRequest.ReactionsRequestBuilder()
        ...
        if (paginationTask == 0){
            reactionRequest.fetchNext(object : CallbackListener<List<Reaction>>(){
                override fun onSuccess(messageReactions: List<Reaction>) {
            ...
            reactionRequest.fetchPrevious(object : CallbackListener<List<Reaction>>(){
                override fun onSuccess(messageReactions: List<Reaction>) {
                    val list = messageReactions.map { e -> getMessageReactionMap(e) }
                    val resultMap = mutableMapOf<String, Any?>()
                    resultMap["key"] = key
    ...
    private fun fetchPreferences(call: MethodCall, result: Result){
        CometChatNotifications.fetchPreferences(object : CallbackListener<NotificationPreferences>() {
            override fun onSuccess(notificationPreferences: NotificationPreferences) {
                result.success(notificationPreferences.toMap())
            }
    ...
    private fun updatePreferences(call: MethodCall, result: Result){
        val raw = call.arguments
        val requestObj = NotificationPreferences.fromMap(raw as MutableMap<String, Any>?)
        CometChatNotifications.updatePreferences(requestObj, object : CallbackListener<NotificationPreferences>() {
            override fun onSuccess(notificationPreferences: NotificationPreferences) {
                result.success(notificationPreferences.toMap())
            }
    ...
    private fun resetPreferences(call: MethodCall, result: Result){
        CometChatNotifications.resetPreferences(object : CallbackListener<NotificationPreferences>() {
            override fun onSuccess(notificationPreferences: NotificationPreferences) {
                result.success(notificationPreferences.toMap())
            }
    ...
    private fun getMutedConversations(call: MethodCall, result: Result){
        CometChatNotifications.getMutedConversations(object : CallbackListener<List<MutedConversation>>() {
            override fun onSuccess(mutedConversations: List<MutedConversation>) {
                val r1 : MutableMap<Int, MutableMap<String, Any>> = mutableMapOf()
                for ((index, value) in mutedConversations.withIndex()) {
                    r1[index] = mutedConversations[index].toMap()
                }
                result.success(r1)
            }
    ...
    private fun muteConversations(call: MethodCall, result: Result){
        val raw = call.arguments
        val requestObj = raw as MutableMap<*, *>?
        val mutedConversation: MutableList<MutedConversation> = arrayListOf()
        requestObj?.forEach { (key, value) ->
            val mutedConversationMap = value as Map<String, Any>?
            val mutedConversationObj = MutedConversation.fromMap(mutedConversationMap)
            mutedConversation.add(mutedConversationObj)
        }
        CometChatNotifications.muteConversations(mutedConversation, object : CallbackListener<String>() {
    ...
    private fun unmuteConversations(call: MethodCall, result: Result){
        val raw = call.arguments
        val requestObj = raw as MutableMap<*, *>?
        val unmuteConversations: MutableList<UnmutedConversation> = arrayListOf()
        requestObj?.forEach {(key, value) ->
            val unmuteConversationMap = value as Map<String, Any>?
            unmuteConversations.add(UnmutedConversation.fromMap(unmuteConversationMap))
        }
        CometChatNotifications.unmuteConversations(unmuteConversations, object : CallbackListener<String>() {
            override fun onSuccess(response: String?) {
                result.success(response)
            }
    ...
    private fun registerPushToken(call: MethodCall, result: Result){
        val pushPlatforms: PushPlatforms = PushPlatforms.get(call.argument("pushPlatforms"))
        val fcmToken: String = call.argument("fcmToken") ?: ""
        val providerId: String? = call.argument("providerId")
        CometChatNotifications.registerPushToken(fcmToken, pushPlatforms, providerId, object : CallbackListener<String>() {
            override fun onSuccess(response: String) {
                result.success(response)
            }
    ...
    private fun unregisterPushToken(call: MethodCall, result: Result){
        CometChatNotifications.unregisterPushToken(object : CallbackListener<String>() {
            override fun onSuccess(response: String) {
                result.success(response)
            }
    ...
    private fun getConversationUpdateSettings(call: MethodCall, result: Result){
        val conversationUpdateSettings: ConversationUpdateSettings = CometChat.getConversationUpdateSettings()
        result.success(conversationUpdateSettings.toMap())
    }

    private fun updateTimezone(call: MethodCall, result: Result){
        val timezoneIdentifier: String = call.argument("timezone") ?: ""

            val timezone = TimeZone.getTimeZone(timezoneIdentifier)
            if (timezone == null) {
                result.error("ERROR", "Invalid timezone", "Invalid timezone identifier received")
                return
            } else {
    ...
    private fun getTimezone(call: MethodCall, result: Result){
        CometChatNotifications.getTimezone(object : CallbackListener<TimeZone>() {
            override fun onSuccess(response: TimeZone) {
